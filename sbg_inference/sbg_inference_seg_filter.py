...
def parse_args():
    ...
    p.add_argument('--bbox_xy_pad', type=float, default=0.02,
...
def pose_server():
    """TCP-сервер, отдает по кругу одну строку current_line."""
    ...
    while True:
        ...
        with conn:
            ...
            try:
                ...
                print(f"[POSE-SERVER] connection from {addr}, sending 1 pose")
            except Exception as e:
                print(f"[POSE-SERVER] send error: {e}")
        time.sleep(0.1)
# ----------------------------------------------------------------------
# MAIN
# ----------------------------------------------------------------------
def main():
    ...
    server_thread = threading.Thread(target=pose_server, daemon=True)
    server_thread.start()

    # ====== INIT SBG, YOLO, RealSense, Visualizer ======
    net = GraspNet_MSCQ(
    ...
    seg_model = YOLOv8Seg(
        onnx_model='/home/nikita/.../best.onnx',
        yaml_path ='/home/nikita/.../merged_yolo_dataset2.yaml')
    ...
    try:
        while True:
            ...
            # ── Segmentation ─────────────────────────────────────────
            boxes, segments, _ = seg_model(color_img, 0.4, 0.5)
            ...
            xyz_s, _ = sample_point_cloud(
                xyz_full, color_full.astype(np.float32)/255, 500)
            if xyz_s is None:
                continue
            ...
            # ── Формируем новый список pose_lines из gg_offset ────
            if gg_offset.shape[0] > 0:
                row  = gg_offset[0]
                t    = row[13:16]
                R    = row[4:13].reshape(3, 3)
                vals = np.concatenate([t, R.flatten()])
                new_line = " ".join(f"{v:.6f}" for v in vals) + "\n"
