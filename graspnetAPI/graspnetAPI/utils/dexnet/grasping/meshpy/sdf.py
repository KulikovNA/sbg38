...
class Sdf3D(Sdf):
    ...
    def _signed_distance(self, coords):
        ...
        if self.is_out_of_bounds(coords):
            ...
            if np.issubdtype(type(coords[0]), integer) and \
               np.issubdtype(type(coords[1]), integer) and \
               np.issubdtype(type(coords[2]), integer):
    ...
    def transform_dense(self, delta_T, detailed=False):
        ...
        if detailed:
            ...
            pts_tf_round = np.round(pts_tf).astype(int64)
