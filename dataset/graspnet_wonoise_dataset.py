...
class GraspNetDataset(Dataset):
    ...
    def augment_data(self, point_clouds, object_poses_list):
        ...
        if np.random.random() > 0.5:
            ...
            for i in range(len(object_poses_list)):
                object_poses_list[i] = np.dot(flip_mat, object_poses_list[i]).astype(float32)
        ...
        for i in range(len(object_poses_list)):
            object_poses_list[i] = np.dot(rot_mat, object_poses_list[i]).astype(float32)
    ...
    def get_data(self, index):
        ...
        ret_dict = {}
        ret_dict['point_clouds'] = cloud_sampled.astype(float32)
        #ret_dict['cloud_colors'] = color_sampled.astype(float32)
        return ret_dict
    ...
    def get_data_label(self, index):
        ...
        try:
            obj_idxs = meta['cls_indexes'].flatten().astype(int32)
            poses = meta['poses']
        ...
        # for i in range(len(object_poses_list)):
        #     object_poses_list[i] = np.dot(trans[:3,:3], object_poses_list[i]).astype(float32)

        ret_dict['point_clouds'] = cloud_sampled.astype(float32)
        ret_dict['objectness_label'] = objectness_label.astype(int64)
    ...
    def get_data_label_noise(self, index):
        color = np.array(Image.open(self.colorpath[index]), dtype=float32) / 255.0
...
class GraspNetSegDataset(GraspNetDataset):
    def get_data_label(self, index):
        ...
        if self.augment:
            cloud_sampled, object_poses_list, aug_trans = self.augment_data(cloud_sampled, object_poses_list)
            ret_dict['aug_trans'] = aug_trans
        offsets = np.zeros((len(seg_sampled), 3), dtype=float32)
        cf_3D_centers = np.zeros((100, 3), dtype=float32)  # 100 max object centers
        for i, k in enumerate(np.unique(seg_sampled)):
            ...
            # Compute directions
            object_center_offsets = (center - cloud_sampled).astype(float32)
            offsets[mask, ...] = object_center_offsets[mask, ...]

        foreground_mask = (seg_sampled > 0).astype(int64)
        ret_dict['point_clouds'] = cloud_sampled.astype(float32)
        ret_dict['objectness_label'] = objectness_label.astype(int64)
        ret_dict['object_poses_list'] = object_poses_list
        ret_dict['grasp_points_list'] = grasp_points_list
...
class GraspNetDataset_Align(Dataset):
    ...
    def get_data(self, index, return_raw_cloud=False):
        ...
        ret_dict['point_clouds'] = cloud_sampled.astype(float32)
        ret_dict['cloud_colors'] = color_sampled.astype(float32)
        ret_dict['clear_point_clouds'] = clear_cloud_sampled.astype(float32)
...
class GraspNetDataset_mix(GraspNetDataset_Align):
    ...
    def get_data_label(self, index):
        ...
        ret_dict['point_clouds'] = cloud_sampled.astype(float32)
        ret_dict['noise_point_clouds'] = noise_cloud_sampled.astype(float32)
        ret_dict['clear_point_clouds'] = clear_cloud_sampled.astype(float32)
        ret_dict['objectness_label'] = objectness_label.astype(int64)
        ret_dict['object_poses_list'] = object_poses_list
        ret_dict['grasp_points_list'] = grasp_points_list
...
def load_grasp_labels(root):
    ...
    for i, obj_name in enumerate(tqdm(obj_names, desc='Loading grasping labels...')):
        ...
        grasp_labels[i + 1] = (label['points'].astype(float32), label['offsets'].astype(float32),
                                label['scores'].astype(float32), tolerance)
